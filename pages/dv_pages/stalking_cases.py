...
st.markdown("<h1 style='text-align: center;'>Prosecuting Stalking Cases</h1>", unsafe_allow_html=True)

with st.expander("Stalking Cases - Data Notes", expanded=False, icon="📝"):
    notes_text = Path("assets/text/dv_pages/stalking.txt").read_text(encoding="utf-8")
...
st.markdown("<h4 style='text-align: center;'>Stalking Cases Processed Year-to-Date</h4>", unsafe_allow_html=True)
st.write(" ")
...
def disp_outcomes(disp: pd.DataFrame = DISP, date_col: str = "earliest_disp_date"):
    ...
    disp_dict = {
        ...
        2: 'Guilty Plea',
        3: 'Guilty Plea', # 'Plea Deal', # essentially a Guilty Plea
        4: 'Diversion',
...
def file_rate(rcvd: pd.DataFrame = RCVD, fld: pd.DataFrame = FLD, ntfld: pd.DataFrame = NTFLD, disp: pd.DataFrame = DISP, date_col: str = "ref_date"):
    ...
    # Filter DV for only those with DV assaults
    rcvd = rcvd.loc[rcvd['stalking'], ["pbk_num", "ref_date"]]
    ...
    # Current file rate 
    # Get counts for both statuses at once
    status_counts = filtered_df[filtered_df["status"].isin(["Filed", "Not Filed"])].groupby("status")["count"].sum()

    fld_cases = status_counts.get("Filed", 0)
    ntfld_cases = status_counts.get("Not Filed", 0)
    total_cases = fld_cases + ntfld_cases
